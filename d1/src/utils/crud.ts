...
import {ColumnMap, parseFilters, parseInclude, parsePagination, parseSort} from "./helper";
...
export function createCrudRoutes<T extends AnySQLiteTable>({
    ...
    return async (req: Request, env: any) => {
        ...
        if (req.method === "GET") {
            ...
            // If includes are requested, try to use queryMap
            ...
            // Default: no includes → use generic select
            if (id && id !== tableName) {
                const rows = await db
                    .select()
                    .from(table)
